<div x-data="{ open: false }">
   ...
   <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 ">
    ...
    <button @click="open = true" data-dialog-target="sign-in-modal"
        ...
        <div class="bg-white rounded-lg shadow-lg w-[730px] p-6">
            ...
            <div x-data="{
               ...
               async enviarFormulario() {
                    ...
                    if (this.archivo && !['image/jpeg', 'image/png'].includes(this.archivo.type)) {
                        this.errores.archivo = 'Solo se permiten JPG, PNG .';
                    } else if (this.archivo && this.archivo.size > 2 * 1024 * 1024) {
                        this.errores.archivo = 'El archivo no puede superar los 2MB.';
                    }
                    ...
                    if (Object.keys(this.errores).length === 0) {
                        try {
                            let response = await fetch('{{ route('cliente.store') }}', {
                            method: 'POST',
                            headers: {
                ...
                <div class="flex justify-end space-x-2">
                    <form @submit.prevent="enviarFormulario" action="{{ route('cliente.store') }}" class="space-y-6"
                                        ...
                                        <div class="mt-4 flex text-sm/6 text-gray-600">
                                            <label for="archivo"
                                                ...
                                                <input id="archivo" type="file" name="archivo" class="sr-only"
                                                    @change="handleFile" />
                                                <p x-text="errores.archivo" class="text-red-500 text-sm"></p>
                                            </label>
